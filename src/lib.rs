...
impl<F: AsRawFd> Master<F> {
	...
	-> impl Future<Item = (SmallVec<M>, Self), Error = io::Error> where
		A: Array<Item = u8>,
	...
	-> impl Future<Item = Self, Error = io::Error> where
		A: Array<Item = u8>,
	{
...
impl<F: AsRawFd> Master<F> {
	...
	-> impl Future<Item = (u8, Self), Error = io::Error> {
	...
	-> impl Future<Item = (u16, Self), Error = io::Error> {
	...
	-> impl Future<Item = (SmallVec<B>, Self), Error = io::Error> where
		A: Array<Item = u8>,
		B: Array<Item = u8>,
	{
...
struct FromPathFuture<P: AsRef<Path>> {
	path: Option<P>,
}

impl<P: AsRef<Path>> Future for FromPathFuture<P> {
...
struct SetRetriesFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	retries: usize,
}

impl<F: AsRawFd> Future for SetRetriesFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SetTimeoutFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	duration: Duration,
}

impl<F: AsRawFd> Future for SetTimeoutFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SetSlaveAddressFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	address: u16,
}

impl<F: AsRawFd> Future for SetSlaveAddressFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct TransferFuture<F: AsRawFd, A, M> where
	A: Array<Item = u8>,
	M: Array<Item = Message<A>>,
{
...
impl<F: AsRawFd, A, M> Future for TransferFuture<F, A, M> where
	A: Array<Item = u8>,
	M: Array<Item = Message<A>>,
{
...
struct ReadBlockDataFuture<F: AsRawFd, A> where
	A: Array<Item = u8>,
{
...
impl<F: AsRawFd, A> Future for ReadBlockDataFuture<F, A> where
	A: Array<Item = u8>,
{
...
struct WriteBlockDataFuture<F: AsRawFd, A> where
	A: Array<Item = u8>,
{
	item: Option<(SmallVec<A>, Master<F>)>,
	command: u8,
...
impl<F: AsRawFd, A> Future for WriteBlockDataFuture<F, A> where
	A: Array<Item = u8>,
{
	type Item = Master<F>;
	type Error = io::Error;
...
struct ReadFuture<F: AsRawFd + Read, A> where
	A: Array<Item = u8>,
{
	item: Option<(SmallVec<A>, Master<F>)>,
	exact: bool,
...
impl<F: AsRawFd + Read, A> Future for ReadFuture<F, A> where
	A: Array<Item = u8>,
{
	type Item = (SmallVec<A>, Master<F>);
	type Error = io::Error;
...
struct WriteFuture<F: AsRawFd + Write, A> where
	A: Array<Item = u8>,
{
	item: Option<(SmallVec<A>, Master<F>)>,
	exact: bool,
...
impl<F: AsRawFd + Write, A> Future for WriteFuture<F, A> where
	A: Array<Item = u8>,
{
	type Item = (SmallVec<A>, Master<F>);
	type Error = io::Error;
...
struct SmbusSetPecFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	pec: bool,
}

impl<F: AsRawFd> Future for SmbusSetPecFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusWriteQuickFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	read_bit: bool,
}

impl<F: AsRawFd> Future for SmbusWriteQuickFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusReadByteFuture<F: AsRawFd> {
	item: Option<Master<F>>,
}

impl<F: AsRawFd> Future for SmbusReadByteFuture<F> {
...
struct SmbusWriteByteFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	byte: u8,
}

impl<F: AsRawFd> Future for SmbusWriteByteFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusReadByteDataFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	command: u8,
}

impl<F: AsRawFd> Future for SmbusReadByteDataFuture<F> {
	type Item = (u8, Master<F>);
	type Error = io::Error;
...
struct SmbusWriteByteDataFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	command: u8,
	byte: u8,
}

impl<F: AsRawFd> Future for SmbusWriteByteDataFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusReadWordDataFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	command: u8,
}

impl<F: AsRawFd> Future for SmbusReadWordDataFuture<F> {
...
struct SmbusWriteWordDataFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	command: u8,
	word: u16,
}

impl<F: AsRawFd> Future for SmbusWriteWordDataFuture<F> {
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusProcessCallFuture<F: AsRawFd> {
	item: Option<Master<F>>,
	command: u8,
	word: u16,
}

impl<F: AsRawFd> Future for SmbusProcessCallFuture<F> {
	type Item = (u16, Master<F>);
	type Error = io::Error;
...
struct SmbusReadBlockDataFuture<F: AsRawFd, A> where
	A: Array<Item = u8>,
{
	item: Option<(SmallVec<A>, Master<F>)>,
	command: u8,
...
impl<F: AsRawFd, A> Future for SmbusReadBlockDataFuture<F, A> where
	A: Array<Item = u8>,
{
	type Item = (SmallVec<A>, Master<F>);
	type Error = io::Error;
...
struct SmbusWriteBlockDataFuture<F: AsRawFd, A> where
	A: Array<Item = u8>,
{
	item: Option<(SmallVec<A>, Master<F>)>,
	command: u8,
...
impl<F: AsRawFd, A> Future for SmbusWriteBlockDataFuture<F, A> where
	A: Array<Item = u8>,
{
	type Item = Master<F>;
	type Error = io::Error;
...
struct SmbusBlockProcessCallFuture<F: AsRawFd, A, B> where
	A: Array<Item = u8>,
	B: Array<Item = u8>,
{
...
impl<F: AsRawFd, A, B> Future for SmbusBlockProcessCallFuture<F, A, B> where
	A: Array<Item = u8>,
	B: Array<Item = u8>,
{
